...
public class AoC3 {
    ...
    public static void main(String[] args) {           
        ...
        int vzdalenost = 0;
        int[] a = {0,0,0};
        for(int i = 0; i < posun1.length; i++) { 
            strana = posun1[i].charAt(0);
            vzdalenost = Integer.parseInt(posun1[i].substring(1));
            if(strana == 'R') {
                for(int y = 0; y < vzdalenost; y++) {
                    int[]souradnice = {a[0]+=1,a[1],a[2]+=1};
                    body.add(souradnice);
                }
            }
            else if(strana == 'L') {
                ...
                for(int y = 0; y < vzdalenost; y++) {
                    int[]souradnice = {a[0],a[1]+=1,a[2]+=1};
                    body.add(souradnice);
                }
            }
            else if(strana == 'D') {
        ...
        ArrayList<int[]> body2 = new ArrayList<int[]>();
        char strana2 = ' ';
        int vzdalenost2 = 0;
        int[] a2 = {0,0,0};
        for(int i = 0; i < posun2.length; i++) { 
            strana = posun2[i].charAt(0);
            vzdalenost2 = Integer.parseInt(posun2[i].substring(1));
            if(strana == 'R') {
                for(int y = 0; y < vzdalenost2; y++) {
                    int[]souradnice = {a2[0]+=1,a2[1],a2[2]+=1};
                    body2.add(souradnice);                    
                }
            }
            else if(strana == 'L') {
                for(int y = 0; y < vzdalenost2; y++) {
                ...
                for(int y = 0; y < vzdalenost2; y++) {
                    int[]souradnice = {a2[0],a2[1]+=1,a2[2]+=1};
                    body2.add(souradnice);
                }
            }
            else if(strana == 'D') {
        ...
        for(int[] x : body) {
            for(int[] y : body2) {
                if((x[0] == y[0])&&(x[1] == y[1])) {
                    System.out.println("[" + (x[0] + x[1]) + "]" + ", Kroky: " + (x[2] + y[2]));
                }
            }
        }
        //for(int[] x : body) {
        //        System.out.println("[" + x[0] + "," + x[1] + "]"  + ", Kroky: " + x[2]);               
        //}
    }    
}
